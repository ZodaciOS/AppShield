...
JAILBREAK_STRINGS = (
    ...
    b"jbexec", b"launchctl", b"spawn" # Also related imports
)
SUSPICIOUS_IMPORTS = (
    ...
    b"LSApplicationWorkspace", b"SpringBoardServices", b"SBS", # Prefixes for SpringBoard Services
    b"UIWebView", b"objc_getClass", b"class_getInstanceMethod", b"method_exchangeImplementations" # Hooking related
)
...
SUSPICIOUS_FILE_EXTS = (".zip", ".rar", ".db", ".sql", ".sqlite") # Suspicious if in root
...
AWS_KEY_REGEX = re.compile(rb'(A3T[A-Z0-9]|AKIA|AGPA|AROA|AIPA|ANPA|ANVA|ASIA)[A-Z0-9]{16}') # More comprehensive AWS Key pattern
        ...
        try:
            ...
            with zipfile.ZipFile(self.ipa_path, "r") as z:
                ...
                for zinfo in z.infolist():
                    ...
                    if "swiftui.framework" in lower or "_concurrency" in lower: has_swiftui = True # Broaden SwiftUI check slightly
                    ...
                    # Check for suspicious files in app root
                    if "/" not in rel_path and any(rel_path.endswith(ext) for ext in SUSPICIOUS_FILE_EXTS):
                        self.details["suspicious_files"].append(rel_path)
                        self._add_finding("Suspicious File", f"Found '{rel_path}' in app root", 2)
                    ...
                    elif lower.endswith((".js", ".html")): # Also scan JS/HTML for strings
                ...
                for k in ["extracted_urls", "extracted_ips", "jailbreak_strings", "suspicious_imports", "weak_hashes", "suspicious_ddns", "tracking_libs", "crypto_libs", "hardcoded_aws_keys", "suspicious_files"]: self.details[k] = sorted(list(set(self.details[k])))
...
class AppUI:
    def __init__(self):
        ...
        self.score_label = ttk.Label(top_frame, text="Risk: N/A", font=self.default_font_bold, style="TLabel") # Use bold font
        self.score_label.pack(side="right", padx=(10, 0))

        self.main_pane = ttk.PanedWindow(self.root, orient="horizontal")
        self.main_pane.pack(fill="both", expand=True, padx=12, pady=(0, 8))

        left_frame = ttk.Frame(self.main_pane) # Removed width for PanedWindow
        ttk.Label(left_frame, text="Findings", font=self.default_font_bold, style="TLabel").pack(anchor="w", pady=(0, 5), padx=(0,0)) # Added padx
        ...
        self.tree.column("info", width=250, stretch=True, anchor="w") # Let info column stretch
        
        tree_scroll = ttk.Scrollbar(left_frame, orient="vertical", command=self.tree.yview)
        self.tree.configure(yscrollcommand=tree_scroll.set)
        tree_scroll.pack(side="right", fill="y", padx=(0,0)) # Added padx
        self.tree.pack(side="left", fill="both", expand=True, padx=(0,0)) # Added padx
        ...
        self.main_pane.add(left_frame, weight=35) # Adjusted weight slightly

        right_frame = ttk.Frame(self.main_pane) # Removed width
        ...
        search_entry.pack(fill="x", padx=1, pady=(2,1)) # Consistent padding
        ...
        self.notebook.pack(fill="both", expand=True, pady=(0,0)) # Adjusted padding
    ...
    def setup_font_and_style(self):
        ...
        style.configure("Treeview.Heading", font=self.default_font_bold, background=INACTIVE_BG, foreground=self.FG, padding=(8, 8)) # Increased heading padding
        style.map("Treeview.Heading", background=[('active', SELECT_BG)])
        style.configure("TNotebook", background=BG, borderwidth=0, padding=(0, 5))
        style.configure("TNotebook.Tab", background=INACTIVE_BG, foreground=INACTIVE_FG, padding=(14, 7), font=(self.default_font_name, default_size)) # Increased tab padding
        style.map("TNotebook.Tab", background=[('selected', BG)], foreground=[('selected', self.FG)])
    ...
    def _show_progress_dialog(self):
        ...
        prog_win_width = 600; prog_win_height = 400 # Define size
        ...
        main_win_x = self.root.winfo_x(); main_win_y = self.root.winfo_y()
        main_win_width = self.root.winfo_width(); main_win_height = self.root.winfo_height()
        ...
        pb.pack(fill="x", padx=10, pady=5); pb.start(10)
        ...
        log_scroll.config(command=self.log_text_widget.yview); log_scroll.pack(side="right", fill="y"); self.log_text_widget.pack(fill="both", expand=True)
