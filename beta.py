...
class AppUI:
    ...
    def show_image_viewer(self, path):
        ...
        try:
            ...
            label = ttk.Label(win, image=photo); label.image = photo; label.pack(padx=10, pady=10)
    ...
    def get_file_hash(self):
        if not self.selected_file_path: return
        try: h = sha256_of_file(self.selected_file_path); content = f"File: {os.path.basename(self.selected_file_path)}\n\nSHA256: {h}"; self.show_text_viewer(self.selected_file_path, content_override=content, title_prefix="SHA256 Hash")
